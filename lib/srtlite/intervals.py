...
class GenomeIntersector(object):
    ...
    def add(self, chrom, start, end, name):
        try:
            self.intersectors[chrom].add(int(start), int(end), value=name)
        except KeyError:
            self.intersectors[chrom] = Intersector()
            self.intersectors[chrom].add(int(start), int(end), value=name)

    def add_feature(self, feature):
        """Add an interval from a Feature object"""
        self.add(feature.chrom, feature.start, feature.end, feature.name)

    def add_features(self, features):
        for f in features:
            self.add(f.chrom, f.start, f.end, f.name)

    def add_from_string(self, line, name):
        for delim in [":", "-"]:
            line = line.replace(delim, " ")
        tokens = line.split()
        self.add(tokens[0], int(tokens[1]), int(tokens[2]), name)

    def find(self, chrom, start, end):
        try:
            return self.intersectors[chrom].find(start, end)
        except KeyError:
            return []
    ...
    def __repr__(self):
        output = []
        for chrom in self.intersectors:
            for interval in self.intersectors[chrom].intervals:
                output.append("%s:%i-%i %s" % (chrom, interval.start, interval.end, str(interval.value)))
        return "\n".join(output)
